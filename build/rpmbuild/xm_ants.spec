...
#make %{?_smp_mflags}
make cleanall
make ants release=%{version}
#make


%install
rm -rf %{buildroot}
make install-xm_ants DESTDIR=%{buildroot}%{prefix}
...
sh %{prefix}/%{name}/scripts/add_cron.sh xm_ants

%preun
chmod +x %{prefix}/%{name}/scripts/remove_cron.sh
sh %{prefix}/%{name}/scripts/remove_cron.sh xm_ants
service %{name} stop
