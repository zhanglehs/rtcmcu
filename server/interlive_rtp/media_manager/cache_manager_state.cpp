...
namespace media_manager
{
  ...
  void cache_manager_state_check_handle(HTTPConnection* conn, void* arg)
  {
    ...
    cache_manager_state((char *)(request->uri.c_str()), NULL, rsp);
  ...
  {
    CacheManager::get_cache_manager()->http_state(query, param, rsp);
  }

  void CacheManager::http_state(char* query, char* param, json_object* rsp)
  {
    char buf[128];
  ...
  void CacheManager::set_http_server(http::HTTPServer *server)
  {
  ...
  void CacheManager::_init_http_server()
  {
    _http_handler_map.clear();

    _add_http_handler("cache", NULL, &CacheManager::_state);
    _add_http_handler("stream_list", NULL, &CacheManager::_stream_list);
    _add_http_handler("store", "stream", &CacheManager::_store_state);
    _add_http_handler("whitelist_fake", NULL, &CacheManager::_whitelist_fake);
  }

  void CacheManager::_add_http_handler(const char* query, const char* param, HttpHandler_t handler)
  {
    _http_handler_map[string(query)] = handler;
  }

  void CacheManager::_stream_list(char* query, char* param, json_object* rsp)
  {
  ...
  void CacheManager::_state(char* query, char* param, json_object* rsp)
  {
    json_object* store_map_size = json_object_new_int(_stream_store_map.size());
    json_object_object_add(rsp, "stream_store_map_size", store_map_size);
  ...
  void CacheManager::_store_state(char* query, char* param, json_object* rsp)
  {
  ...
  void CacheManager::_whitelist_fake(char* query, char* param, json_object* rsp)
  {
  ...
  void CacheManager::_live_cache_state(CircularCache* cache, json_object* rsp)
  {
