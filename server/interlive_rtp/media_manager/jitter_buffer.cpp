...
namespace media_manager
{
    ...
    void JitterBuffer::SetRTPPacket(fragment::RTPBlock *rtpblock) {
        if (_time_base == 0)
        {
            DBG("SetRTPPacket  skipped,timebased 0 payload %d ssrc %d", rtpblock->get_payload_type(), rtpblock->get_ssrc());
            return;
        }
        ...
        {
          if (abs(int(rtpblock->get_timestamp() - _last_queue_block->rtp_time))
            >= int(20 * _time_base)) {
            _reset();
    ...
    fragment::RTPBlock *JitterBuffer::GetRTPPacket() {
        ...
        if (_state == SENDING) {
            ...
            while (!_queue.empty())
            {
                ...
                if (
                    *(_jitter_block) != *(_last_queue_block)
                ...
                {
                    //_jitter_block->update(_queue.empty() ? NULL : _queue.top());
                    if (_lost_queue.empty())
                    {
