    ...
    def __recv_worker(self):
	   ...
	   try:
               ...
               if None != self.__callback_pfn and None != line:
                  self.__callback_pfn(self.split(line))
               if line == '':
	          time.sleep(0.005)
	   except Exception as e:
	       print('__recv_worker, raised Exception:' + str(e))
...
if __name__ == "__main__": 
    rpt = ReportCriticalInfo("/tmp/rpt_critical_info_fifo")
    rpt.create(True)
    rpt.recv(callback_recv_msg)
    while True:
        #rpt.send('lctrl\trpt\tstreamid\tchnid\n')
        time.sleep(0.5)
    rpt.close()
