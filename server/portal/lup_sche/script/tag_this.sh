...
svn_tag_root="http://gforge.1verge.net/svn/interlive/tags/lup_sche"
cwd=$(pwd)
...
make_tag() {
    cd ..
    cwd_new=$(pwd)
    src_path_new=$(dirname $cwd_new) 
	tag_name="lup_sche_$new_ver_str"
	svn copy $src_path_new $svn_tag_root/$tag_name -m "add tag for $new_ver_str"
