...
int tracker_parse_config()
{
    ...
    q = xmlgetattr(server, "network_type");
    config->network_type = 0;
    if (!q)
    {
        WRN("network_type get failed.");
    }
    else
    {
    ...
    for (; ; cnt++)
    {
        ...
        q = xmlgetattr(forward_set, "group_id");
        if (!q)
        {
            WRN("group_id get failed.");
            fs->group_id = -1;
        }
        else
        {
...
int
tc_get_group_id(uint32_t ip, uint32_t *group_id)
{
    list_head_t *pos;
    list_for_each(pos, &(g_forwards_config->root))
    {
...
int
tc_get_forward_fp_set(std::vector<server_info>& forward_fp_set, uint16_t asn)
{
    list_head_t *pos;
    ...
    list_for_each(pos, &(g_forwards_config->root))
    {
        ...
        if (1 == fs->topo_level && asn == fs->asn)
        {
            while (ip <= fs->stop_ip)
            {
                ...
                DBG("tc_get_forward_fp_set:asn: %d, forward_fp_set.push_back:%s", asn, item.name.c_str());
                forward_fp_set.push_back(item);
                ip++;
            }
        }
        else if (1 == fs->topo_level && asn != fs->asn)
        {
            while ( ip <= fs->stop_ip )
            {
                /* return alias ip if there exist */
                const char * alias = tc_get_alias(ip);
                DBG("tc_get_forward_fp_set:asn != fs->asn, asn %u, fs->asn %u, ip %s, alias %p", asn, fs->asn, util_ip2str(ip), alias);
                if (alias)
                {
                    const char * s = alias, *e = alias;
                    while ((e = strchr(s, ';')) != NULL)
                    {
                        ...
                        if (asn == alias_asn)
                        {
                            ...
                            DBG("tc_get_forward_fp_set:forward_fp_set.push_back:%s", item.name.c_str());
                            forward_fp_set.push_back(item);
                            break;
                        }
                        s = e + 1;
