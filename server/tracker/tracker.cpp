...
static int tracker_process_r2p_keepalive(client_t* c);
static int tracker_process_f2p_white_list_req(client_t* c);
static int tracker_process_f2p_keepalive(client_t* c);
static int tracker_process_st2t_fpinfo_req(client_t* c);
// mt2t version
...
{
    tracker_broadcast_stream_start(item);
    g_stream_list_buffer_stale = true;
}

void stream_close_cb(uint32_t streamid)
{
    tracker_broadcast_stream_close(streamid);
    g_stream_list_buffer_stale = true;
}
...
tracker_timer_handler(int fd, short which, void *arg)
{
    ...
    {
        g_portal->keepalive();
        forward_manager->fm_keep_update_forward_fp_infos();
    }
...
{
    while (buffer_data_len(c->rb)) {
        DBG("tracker_handle_income_data: len(c->rb) = %lu, c->fd = %d", buffer_data_len(c->rb), c->fd);
        ...
        DBG("tracker_handle_income_data: len(c->rb) = %lu, head.size = %u, c->fd = %d", buffer_data_len(c->rb), pro_head.size, c->fd);
        ...
        switch (pro_head.version)
        {
            ...
            case VER_1:
            {
                ...
                switch (pro_head.cmd)
                {
                    case CMD_R2P_KEEPALIVE:
                        DBG("tracker_handle_income_data: CMD_R2P_KEEPALIVE fid = %lu", c->fid);
                        result = tracker_process_r2p_keepalive(c);
                        break;
                    case CMD_F2P_WHITE_LIST_REQ:
                        DBG("tracker_handle_income_data: CMD_F2P_WHITE_LIST_REQ fid = %lu", c->fid);
                        result = tracker_process_f2p_white_list_req(c);
                        break;
                    case CMD_F2P_KEEPALIVE:
                        DBG("tracker_handle_income_data: CMD_F2P_KEEPALIVE fid = %lu", c->fid);
                        // todo: dig further on f2p_keepalive
                        result = tracker_process_f2p_keepalive(c);
                        break;
                    case CMD_ST2T_FPINFO_REQ:
                        DBG("tracker_handle_income_data: CMD_F2P_WHITE_LIST_REQ fid = %lu", c->fid);
                        result = tracker_process_st2t_fpinfo_req(c);
                        break;
                    default:
                        ERR("tracker_handle_income_data error: unrecognized command = %u (proto_header.version = 1)", pro_head.cmd);
                }
            }
            break;
            default:
                ERR("tracker_handle_income_data error: proto_header.version = %u (expect 3 or 1)", pro_head.version);
        }

        if(-2 == result)
        {
            WRN("tracker_handle_income_data error: result == -2, fid: %lu, cmd: %u", c->fid, pro_head.cmd);
            return;
        }
        else if (-1 == result)
        {
            WRN("tracker_handle_income_data error: result == -1, fid: %lu, cmd: %u", c->fid, pro_head.cmd);
            tracker_close_client(c);
            return;
        }
    }
...
init_client_struct(client_t *c)
{
    c->fd = -1;
    c->ip = 0;
    c->rb = buffer_init(1*1024*1024);
    if (NULL == c->rb)
        return -1;
    c->wb = buffer_init(1*1024*1024);
    if (NULL == c->wb)
    {
        buffer_free(c->wb);
        return -1;
...
tracker_process_register_v3(client_t *c)
{
    ...
    else
    {
        ...
        req.asn = asn;
        req.region = region;
        c->fid = forward_manager->fm_add_a_forward_v3(&req, level);
        if (c->fid == uint64_t(-1))
        {
...
tracker_process_addr_req_v3(client_t *c)
{
    ...
    else
    {
        ...
        if (req.asn == ASN_TEL || req.asn == ASN_CNC || req.asn == ASN_CMCC) // for original system
        {
            ret = forward_manager->fm_get_proper_forward_v3(upstream_server,
                                                            (ForwardID)c->fid,
                                                            &req.streamid[0],
                                                            req.ip,
                                                            req.port,
        ...
        rsp.ip = upstream_server.ip;
        rsp.port = upstream_server.backend_port;
        rsp.level = upstream_server.topo_level;
...
tracker_process_register_v4(client_t *c)
{
    ...
    else
    {
        ...
        c->fid = forward_manager->fm_add_a_forward_v4(&req, level);
        if (c->fid == uint64_t(-1))
        {
        ...
        else
        {
            ...
            /* add aliases if there exist */
            int alias_cnt = tc_get_all_alias(req.ip(), NULL);
            if (alias_cnt > 0)
            {
                char **alias = new (std::nothrow) char*[alias_cnt];
                if (NULL == alias)
                {
                    ERR("tracker_process_register_v4:new error!");
                    return -1;
                }

                for (int i=0; i<alias_cnt; i++)
                {
                    alias[i] = new (std::nothrow) char[32];
                    if (NULL == alias[i])
                    {
                        ERR("tracker_process_register_v4:new error!");
                        for (int j=0; j<i; j++)
                    ...
                    memset(alias[i], 0, sizeof(char)*32);
                }
                tc_get_all_alias(req.ip(), alias);

                // const char * alias = tc_get_alias(req.ip());
                // if (alias)
                for (int k=0; k<alias_cnt; k++)
                {
                    const char * s = alias[k], *e = alias[k];
                    while ((e = strchr(s, ';')) != NULL)
                    {
                        char str_ip[32] = {0};
                        uint32_t ip;
...
// this breaks DRY principle, should be fixed after we have finished the portal upgrading plan
// and, this function handles the original message from interlive, the following one handles
// the new message from module_tracker
static int
tracker_process_r2p_keepalive(client_t* c)
{
...
tracker_process_r2p_keepalive_mt2t(client_t* c)
{
    ...
    r2p_keepalive* keepalive = (r2p_keepalive*)(buffer_data_ptr(c->rb));
    size_t stream_count = ntohl(keepalive->stream_cnt);
    DBG("tracker_process_r2p_keepalive_mt2t: got %lu stream(s)", stream_count);
    buffer_eat(c->rb, sizeof(r2p_keepalive));
    for (size_t i = 0; i < stream_count; i++)
    {
        ...
        g_stream_manager->update_if_existed(stream_id, ntohl(stream_status->last_ts),
                                            util_ntohll(stream_status->block_seq));
        buffer_eat(c->rb, sizeof(receiver_stream_status));
        DBG("tracker_process_r2p_keepalive_mt2t: stream_id %032x", stream_id);
    }
...
static int
tracker_process_f2p_white_list_req(client_t* c)
{
    ...
    encode_header(c->wb, CMD_P2F_WHITE_LIST_RSP,
...
// See comment of tracker_process_r2p_keepalive
static int
tracker_process_f2p_white_list_req_mt2t(client_t* c)
{
    ...
    int append = buffer_append(c->wb, tracker_get_stream_list_buffer());
    if (append) {
        ERR("tracker_process_f2p_white_list_req_mt2t: memory low while encoding response messsage, reset connection");
        return -1;
    }
...
static int
tracker_process_f2p_keepalive(client_t* c)
{
...
static int
tracker_process_st2t_fpinfo_req(client_t* c)
{
    ...
    if (!serialized)
    {
        ERR("tracker_process_st2t_fpinfo_req error: failed to serialize protobuf, fid = %lu", c->fid);
        return -1;
    }

    fpinfo_buffer->_end += bytes_to_serialize;
    size_t bytes_to_send = bytes_to_serialize + sizeof(proto_header);
    if (encode_header_v3(c->wb, CMD_T2ST_FPINFO_RSP, bytes_to_send) != 0)
    {
        ERR("tracker_process_st2t_fpinfo_req error: failed to encode CMD_T2ST_FPINFO_RSP, fid = %lu", c->fid);
        return -1;
    }

    if (buffer_append(c->wb, fpinfo_buffer) != 0)
    {
        ERR("tracker_process_st2t_fpinfo_req error: failed to append CMD_T2ST_FPINFO_RSP to write buffer, fid = %lu", c->fid);
        return -1;
    }
...
buffer* tracker_get_stream_list_buffer()
{
    static buffer* stream_list_buffer = buffer_create_max(4096, 20480 * 20 + 20 + 2);
