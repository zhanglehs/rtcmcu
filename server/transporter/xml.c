...
xmlnchild(struct xmlnode *xml, const char *name)
{
	...
	if (xml == NULL || xml->nchild == 0 || name == NULL || name[0] =='\0') return 0;
	for (i = 0; i < xml->nchild; i ++) {
		x = xml->child[i];
		if (x->name && strcmp(x->name, name) == 0) j ++;
	}
	return j;
}
...
xmlgetattr(struct xmlnode *xml, const char *name)
{
	...
	if (xml == NULL || xml->nattr == 0 || name == NULL || name[0] == '\0') return NULL;

	for (i = 0; i < xml->nattr; i ++) {
		xa = xml->attr[i];
		if (xa->name && strcmp(xa->name, name) == 0) return xa->value;
	}
	return NULL;
}
...
static int xml_depth = 0, xml_error = 0, parent_depth = 0, max_parent_depth=0;
...
startElement(void *data, const char *name, const char **atts)
{
	...
	if (parser_parent[xml_depth]->nchild) {
		/* add new child */
		parser_parent[xml_depth]->child = (struct xmlnode **)realloc((void *) parser_parent[xml_depth]->child,
				(parser_parent[xml_depth]->nchild+1) * sizeof(struct xmlnode *));
		if (parser_parent[xml_depth]->child == NULL) {
			printf("%s: %d\n", __FILE__, __LINE__);
			xml_error = XML_ERR_MEMORY;
			return;
		}
		parser_parent[xml_depth]->child[parser_parent[xml_depth]->nchild] = parser_node;
		parser_parent[xml_depth]->nchild ++;
	} else {
		/* first child */
		parser_parent[xml_depth]->child = (struct xmlnode **)calloc(sizeof(struct xmlnode *), 1);
		if (parser_parent[xml_depth]->child == NULL) {
			xml_error = XML_ERR_MEMORY;
			printf("%s: %d\n", __FILE__, __LINE__);
			return;
	...
	for (i = 0, j = 0; atts[i]; i += 2) j ++;
	if (j) {
		parser_node->attr = (struct xmlattribute **)calloc(sizeof(struct xmlattribute *), j);
		...
		parser_node->nattr = j;
		for (i = 0; i < j; i++) {
			xmlattr = (struct xmlattribute *)calloc(sizeof(struct xmlattribute), 1);
			if (xmlattr == NULL) {
				xml_error = XML_ERR_MEMORY;
				printf("%s: %d\n", __FILE__, __LINE__);
				return;
...
setElement(void *data, const char *el, int len)
{
	...
	if (len <= 0 || parser_node == NULL) return;
	...
	if (parser_node->value == NULL) {
		...
		parser_node->value = (char *)realloc((void *)parser_node->value, j+(len-i)+1);
		if (parser_node->value == NULL) {
			xml_error = XML_ERR_MEMORY;
			printf("%s: %d\n", __FILE__, __LINE__);
			return;
...
xmlloadfile(const char *file)
{
	...
	parent_depth = 1;
	max_parent_depth = 10;
	parser_parent = (struct xmlnode **)calloc(sizeof(struct xmlnode **), max_parent_depth);
